...
class RumorService(SessionAppService):
    ...
    def append_for_region(
        ...
        *,
        ...
        the turn summary covers it. When ``min_source_support`` is set, only
        sufficiently-supported existing rumors seed new distortions (FR-H2 /
        BR-H1-7); ``None`` keeps the unrestricted manual behaviour (BR-H1-8)."""
        return self._generate_for_region(
            session,
            region_id,
            self._chain_degrees(session.id, region_id),
